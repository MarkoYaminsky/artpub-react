...
export const CartItem: React.FC<IItemInfo> = (props) => {
  ...
  useEffect(() => {
    getArticleById(JSON.stringify(props.id)).then(data => setCartItemInfo(data))
  }, [props.id])
  ...
  return (
    <div className="cartItem">
      ...
      <span className="info">
        {cartItemInfo?.title}, by {cartItemInfo?.author} {props.quantity !== 1 && `— ${props.quantity}`}
      </span>
      <p className="price">${cartItemInfo?.price}</p>
      <div className="changeQuantity">
        <img src={plus} alt="plus" onClick={() => dispatcher(addItemToCart(props.id))} />
        <img src={minus} alt="minus" onClick={() => dispatcher(removeItemFromCart(props.id))} />
      </div>
    </div>
  );
};
